package edu.isi.bmkeg.kefed.services.extKefed.impl
{
	...
	public class ExtendedKefedServiceImpl extends Actor implements IExtendedKefedService {
		...
		public function retrieveCompleteKefedModelFromUuid(uuid:String):void {
			server.retrieveCompleteKefedModelFromUuid.cancel();
			server.retrieveCompleteKefedModelFromUuid.addEventListener(ResultEvent.RESULT, retrieveCompleteKefedModelFromUuidResultHandler);
			server.retrieveCompleteKefedModelFromUuid.addEventListener(FaultEvent.FAULT, faultHandler);
			server.retrieveCompleteKefedModelFromUuid.send(uuid);
		}
		
		private function retrieveCompleteKefedModelFromUuidResultHandler(event:ResultEvent):void
		{
			var kefedModel:KefedModel = KefedModel(event.result);
			dispatch(new RetrieveCompleteKefedModelResultEvent(kefedModel));
		}	
		...
		public function deleteCompleteKefedModelFromUid(uid:String):void {
			server.deleteCompleteKefedModelFromUid.cancel();
			server.deleteCompleteKefedModelFromUid.addEventListener(ResultEvent.RESULT, deleteCompleteKefedModelResultHandler);
			server.deleteCompleteKefedModelFromUid.addEventListener(FaultEvent.FAULT, faultHandler);
			server.deleteCompleteKefedModelFromUid.send(uid);
		}
